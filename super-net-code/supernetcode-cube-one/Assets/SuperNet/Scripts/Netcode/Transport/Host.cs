...
namespace SuperNet.Netcode.Transport {
	...
	public class Host : IDisposable {
		...
		public Peer Connect(IPEndPoint remote, PeerConfig config, IPeerListener listener, IWritable message = null) {
			...
			// Start connecting
			Task.Factory.StartNew(() => peer.Connect(message), TaskCreationOptions.PreferFairness);
		...
		public Peer Accept(ConnectionRequest request, PeerConfig config, IPeerListener listener) {
			...
			} else if (config.RemotePublicKey != null && config.RemotePublicKey.Length > 0) {
			...
			// Accept the request
			Task.Factory.StartNew(() => peer.Accept(request), TaskCreationOptions.PreferFairness);
