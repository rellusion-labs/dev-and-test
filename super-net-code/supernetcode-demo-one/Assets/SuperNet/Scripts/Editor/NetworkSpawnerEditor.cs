		...
		public override void OnInspectorGUI() {
			...
			if (warningNoPrefab) {
				...
				if (serializedObject.isEditingMultipleObjects) {
					EditorGUILayout.HelpBox("Prefab not set on one of the spawners.", MessageType.Warning);
				} else {
			...
			if (warningPrefabInstance) {
				EditorGUILayout.Space();
				if (serializedObject.isEditingMultipleObjects) {
					EditorGUILayout.HelpBox("Prefab is a scene object on one of the spawners.", MessageType.Warning);
				} else {
